...
class CustomGraph:
    ...
    def __sample_edges(self, mode: str = 'adding_edges', layer_from: int = None, layer_to: int = None,
        ...
        if layer_from and not layer_to:
            layer_to = random.randint(layer_from + 1, max(layers))
        elif layer_to and not layer_from:
            layer_from = random.randint(0, layer_to - 1)
        elif not layer_from and not layer_to:
        ...
        if mode == 'splitting_edges':
            ...
            num_edges = random.choice([
                random.randint(len(nodes_layer_from) // 10, len(nodes_layer_from)),
                random.randint(len(possible_edges) // 50, len(possible_edges) // 3)
            ])
            return random.sample(possible_edges, num_edges), layer_from
        elif mode == 'weights_duplicating_and_edges_removing':
            possible_edges = [(u, v) for u in nodes_layer_from for v in nodes_layer_to if self.graph.has_edge(u, v)]
        ...
        if mode == 'adding_edges':
            num_edges = min(
                max(self.graph.number_of_edges() // 2, random.choice([2500, 5000])),
                random.randint(len(possible_edges) // 40, len(possible_edges) // 2)
            )
        elif mode == 'adding_nodes':
            num_edges = min(
                random.randint(len(nodes_layer_from) // 4, len(nodes_layer_from)),
                len(possible_edges)
            )
        else:
            return [], None
    ...
    def remove_edges(self, iterations: int = None):
        ...
        for i in range(iterations):
            edges_to_remove, _ = self.__sample_edges(mode='weights_duplicating_and_edges_removing')
    ...
    def add_new_weights_duplicates(self, new_groups_num: int, iterations: int = None):
        ...
        for i in range(iterations):
            edges_to_add_to_group, _ = self.__sample_edges(mode='weights_duplicating_and_edges_removing')
            weight_group_to_add = new_group if new_group and i == 0 else random.choice(self.duplicated_weights_groups)
